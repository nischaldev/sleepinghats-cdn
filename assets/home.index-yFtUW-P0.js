import{m as q}from"./mainView.class-C4NLGCWW.js";import{a as V,t as f,s as j,l as S,S as s,b as z,d as N,w as g,e as O,f as $,h as R,P as K,_ as J,A as Q,n as G,C as Y,i as U,j as x,k as _,m as X,o as Z,p as ee,T as te,V as ie,r as ae,q as se,u as oe,x as ne,y as le,z as re,B as de,D as ce,E as me,F as pe,G as ue,H as ge,I as ve}from"./index-B-kZ959M.js";import{A as he,N as be}from"./navbar.component-CsDjYAtY.js";import{eB as we,ej as ye}from"./mftsccs-browser-DqtqSosg.js";import"./vendor-DEYWGvhS.js";import"./codemirror-D1H9tReK.js";import"./eslint4b-BLWGvIOa.js";import"./prettier-ClL2wQep.js";import"./ckeditor-Cw9bcqxY.js";import"./loginModal.service-1Fv7QTmU.js";function fe(){document.addEventListener("keydown",function(i){i.ctrlKey&&i.altKey&&i.key==="d"?(i.preventDefault(),xe()):i.ctrlKey&&i.altKey&&i.key==="c"?(i.preventDefault(),_e()):i.ctrlKey&&i.altKey&&i.key==="r"?(i.preventDefault(),ke()):i.ctrlKey&&i.altKey&&i.key==="f"&&(i.preventDefault(),Se())})}function xe(){const i=document.getElementById("designSplitViewButton"),e=new Event("click",{bubbles:!0,cancelable:!0});i&&i.dispatchEvent(e)}function _e(){const i=document.querySelector(".codeSplitViewButton"),e=new Event("click",{bubbles:!0,cancelable:!0});i&&i.dispatchEvent(e)}async function ke(){await V.synchronizeWidget("Widget Synchronized")}async function Se(){if(document.getElementById("codeBlocksContainer")?.classList.contains("visible")){const e=document.getElementById("widgetButtionCodeViewWrapper"),t="fullscreen";e?.classList.contains(t)?e?.classList.remove(t):e?.classList.add(t)}}function Ce(){const i=document.getElementById("tour-container");i&&i.remove()}async function Te(){window.addEventListener("online",()=>k()),window.addEventListener("offline",()=>k())}const k=()=>{navigator.onLine?f.infoMessageToast("Internet is back. You are online now.",3):(j(),f.infoMessageToast("You are offline - changes will be saved locally.",3))};class Ee{constructor(){this.init()}init(){document.getElementById("wico_btn-toggle-rightPanel")?.addEventListener("click",t=>{this.toggleRightPanel(t)})}toggleRightPanel(e){const t=e?.target,o=document.getElementById("right-aside"),n=document.getElementById("column-right"),l=document.getElementById("midContent"),r=t?.querySelector("span");o&&(o?.classList.contains("d-none")?(o?.classList.remove("d-none"),n?.classList.add("col-md-3"),l?.classList.remove("col-md-12"),l?.classList.add("col-md-9"),r.textContent="start",t.title="Hide panel",t.classList.add("btn-togglePanel-close"),t.classList.remove("btn-togglePanel-open")):(o?.classList.add("d-none"),n?.classList.remove("col-md-3"),l?.classList.remove("col-md-9"),l?.classList.add("col-md-12"),r.textContent="keyboard_tab_rtl",t.title="Show panel",t.classList.remove("btn-togglePanel-close"),t.classList.add("btn-togglePanel-open")))}}class Me{convertingId=0;convertedData;constructor(e){this.convertingId=e}async init(){const e=await we(Number(this.convertingId)),t=e?.boomgpt?.data_type;this.convertedData=await new Promise(async o=>{switch(t){case"full_page":const n=e?.boomgpt?.full_text,r=new DOMParser().parseFromString(n,"text/html"),d=r.body,m=await Promise.all(Array.from(d.querySelectorAll("script[src]")).map((a,c)=>({order:c,url:a.src})));Array.from(d.querySelectorAll("script[src]"))?.forEach(a=>{a.remove()});const v=d.querySelectorAll("script:not([src])");let h="";v.forEach(a=>{h+=a.textContent}),Array.from(d.querySelectorAll("script:not([src])"))?.forEach(a=>{a.remove()});const C=Array.from(r.head.querySelectorAll("link[rel='stylesheet'")).map((a,c)=>({order:c,url:a.href})),T=r.head.querySelectorAll("style");let b="";T.forEach(a=>{b+=a.textContent}),s.staticWidgetTree.html=d.innerHTML,s.staticWidgetTree.css=b,s.staticWidgetTree.library.css=C,s.staticWidgetTree.library.js=m,s.staticWidgetTree.before_render="this.render();",s.staticWidgetTree.after_render=h;break;case"all":const E=e?.boomgpt?.visual_text,u=new DOMParser().parseFromString(E,"text/html"),p=u.body,M=await Promise.all(Array.from(p.querySelectorAll("script[src]")).map((a,c)=>({order:c,url:a.src})));Array.from(p.querySelectorAll("script[src]"))?.forEach(a=>{a.remove()});const L=p.querySelectorAll("script:not([src])");let w="";L.forEach(a=>{w+=a.textContent}),Array.from(p.querySelectorAll("script:not([src])"))?.forEach(a=>{a.remove()});const W=Array.from(u.head.querySelectorAll("link[rel='stylesheet'")).map((a,c)=>({order:c,url:a.href})),A=u.head.querySelectorAll("style");let y="";A.forEach(a=>{y+=a.textContent}),s.staticWidgetTree.html=p.innerHTML,s.staticWidgetTree.css=y,s.staticWidgetTree.library.css=W,s.staticWidgetTree.library.js=M,s.staticWidgetTree.before_render="this.render();",s.staticWidgetTree.after_render=w;break;case"plain_text":const I=e?.boomgpt?.visual_text,H=new DOMParser().parseFromString(I,"text/html").body;s.staticWidgetTree.html=H.innerHTML;break;case"only_images":const B=e?.boomgpt?.images?.map(a=>`
        ...
        `;break;case"sticky_note":const D=e?.boomgpt?.sticky_note,F=new DOMParser().parseFromString(D,"text/html").body;s.staticWidgetTree.html=F.innerHTML;break;case"widget":const P=new Event("loadWidget");await S(P,Number(e?.boomgpt?.widget_id));break}t!=="widget"&&(await z(s.staticWidgetTree),await N(s.staticWidgetTree,!1),g.enableWidgetButtons(),await O()),o("converted")})}}class Re extends q{the_params;page_id;widget_id;_sideNavbar;_tourStatus=!1;_loginStatus=!1;_pagesTSCCSService;_authService;ctowData;constructor(e){super(e),this._sideNavbar=new $,R.navbar=this._sideNavbar,this.the_params=e,this.ctowData=e?.ctow,this.page_id=e?.["page-id"],this.widget_id=e?.["widget-id"],this._pagesTSCCSService=new K,this.setTitle("Home | Widget Conceptualizer"),window._dragService=J,this._authService=Q.getInstance()}async componentDidMount(){if(this.page_id){const t=document.getElementById("midContent"),o=document.getElementById("right-aside"),n=await this._pagesTSCCSService.getPageById(Number(this.page_id));n?.error&&G();const l=n.data?.[`the_${Y.PAGE_COMP_NAME}`],r={type:U.page,data:l,page_id:this.page_id},d=new x(r),m=new _(r);o.innerHTML=await m.getHtml(),t.innerHTML="",d.mount(t)}if(this.widget_id){const t=new Event("loadWidget");await S(t,Number(this.widget_id))}else g.disableWidgetButtons();if(this._sideNavbar.initialize(),X.renderActionHistory(),fe(),await Te(),await Z(),ee(),new te,ie.renderVisualTree(),(await this._authService?.getLoginInfo()).isLoggedIn&&(new he,new Ee,this.ctowData)){const t=document.getElementById("loading-text");t.textContent="Converting to Widget...";const o=JSON.parse(window.atob(this.ctowData));await new Me(Number(o)).init()}}async getHtml(){s.resetWidgetTree(),ae(),se();const e=await this._authService.getLoginInfo(),t=await oe();if(this._tourStatus=e.tourStatus,this._loginStatus=e.isLoggedIn,e&&e.isLoggedIn){window.closeTour=Ce,window.createWidgetModalOpen=ne,window.toggleElementMenu=le,window.editElementModalOpen=re,window.deleteElement=de,window.closeModal=ce,window.createNewElement=me,window.headerFunctions=pe,window.uploadAttachment=ue,window.removeMyWidgetFromPublic=g.removeMyWidgetFromPublic;let o="";this._loginStatus&&!this._tourStatus&&(o=`
        ...
        `);const n=await this._sideNavbar.getHtml(),r=await new x().getHtml(),m=await new _().getHtml();return new ge().init(),`
      <header class="wico_top-header">
        <div class="wico_header-section">
          ...
          <button onclick="openPromptModal(event, 'wico-ai')" class="btn btn-primary rounded-pill btn-sm btn-ai">
            Use AI for Widget
          </button>
        </div>
    ...
    `}else{const n=await new be(t).getHtml();return setTimeout(async()=>{const l=document.getElementById("widgetGeneratorWidgetContainer");l&&await ye(ve.ai_widget,l)},0),`
    ${n}
    <!-- Hero Section -->
