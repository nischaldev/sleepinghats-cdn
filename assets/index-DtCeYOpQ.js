...
`}before_render(){this.render()}after_render(){if(this.the_params){document.getElementById("fspage-save-button").addEventListener("click",Un);const t=this.the_params.data?.the_page_body?.id;if(document.querySelector(".content-target").classList.add("fspage"),!t){Xt();return}this._pageBuilderService.LoadPageWidget(Number(t))}}getHtml(){return window.openSaveWidgetModal=Zn,window.saveWidget=Ai,window.updateWidget=Yn,window.newWidget=qe,window.previewWidget=Br,window.downloadWebsite=rr,window.moveUp=Ee.moveUp,window.moveDown=Ee.moveDown,window.copyElement=Ee.copyElement,window.editWidget=Ee.editWidget,window.openConvertModal=Nr,window.convertToWidget=jr,window.updatePage=zn,window.previewPage=br,window.undoAction=q.undoAction,window.redoAction=q.redoAction,window.saveWidgetToFolderBtn=_r,window.openPromptModal=bo,window.refineAIData=An,`
    ...
    `,document.querySelector("body #app")?.appendChild(i),me("rename-project-dialog"),"Rename Project Modal"}async getProjectPagesListHTML(e){try{const t=await this._projectsService.getProjectById(e),a=t?.data?.the_project?.[`the_project_${se.PROJECT_PAGE_LINKER}`],i=t?.data?.the_project?.the_project_main_page?.[0]?.id;return a?.length?a?.map(o=>{let r=o?.data?.[`the_${W.PAGE_COMP_NAME}`]?.[`the_${W.PAGE_COMP_NAME}_title`]?.[0]?.data?.the_title;const l=i==o.id,d=l?'<span class="material-symbols-outlined home-icon">home</span>':"",u=l?"":`<li class="d-flex align-items-center justify-content-between" 
      ...
      `,document.body.appendChild(t),t.showModal(),t}catch(t){return console.error("Error:",t),""}}}function Cr(n,e=[],t=[]){return`
  <style>
  .accordion-header {
  background: gray;
  color: white;
...
.custom-menu-item {
  ...
  `}async function Ir(n,e){if(n.preventDefault(),!(await _e.getInstance().getLoginInfo()).isAdmin)return"Sorry You Do Not Have Access To Create Element";const a=new FormData(n.target),i=Object.fromEntries(a);console.log(i,"update formValues");const s=await Si(a,e);await Qn("update-element-dialog");const o=await on();if(s){const r=document.getElementById("created-elements");r?!o||o.trim()===""?r.innerHTML=`
      ...
      <!-- START .a-dialog-header  -->
        ...
        <!-- END .a-dialog-body  -->
      </dialog>`}}class $r{createElemenModaltHTML;constructor(){window._dragService=Jo,this.init()}init(){this.createElemenModaltHTML=this.getHtml(),document.querySelector("body #app")?.insertAdjacentHTML("afterend",this.createElemenModaltHTML)}getHtml(){return window.addNewAttribute=Wr,window.cancelAttributeRow=Ci,window.addAttribute=xr,window.removeSelectedAttribute=kr,window.closeElementModal=Qn,`
    ...
    `}}async function me(n){document.getElementById(n).showModal()}async function be(n){const e=document.getElementById(n);e?.querySelector("form")?.reset(),e?.close(),e?.remove()}async function Li(n,e){n?.stopPropagation();const t=document.getElementById(`menu-${e}`),a=t?.classList.contains("show");document.querySelectorAll(".dropdown-menu").forEach(s=>{s.classList.remove("show")}),t&&!a&&t.classList.add("show")}async function Tc(n){new $r,await me(n)}async function Sc(n,e,t){n.stopPropagation(),console.log("modalId",e),Li(n,Number(t)),new Pr(t)}async function Cc(n,e){Li(n,e),confirm("Are you sure you want to delete the element?")&&(await Ce(e),L.successfullToast("The element deleted successfully")),console.warn("element Deleted sucessfully");const a=await on(),i=document.getElementById("created-elements");i?!a||a.trim()===""?i.innerHTML=`
        <li class="list-unstyled">
          <p class="text-center my-3 text-secondary" >No Elements</p>
        </li>
      `:i.innerHTML=a||"":console.error("Elements list container not found in the DOM.")}async function Zn(){Lt(),document.getElementById("save-widget-dialog")?.remove();const e=document.createElement("dialog");return e.setAttribute("id","save-widget-dialog"),e.classList.add("col-md-3"),e.innerHTML=`
