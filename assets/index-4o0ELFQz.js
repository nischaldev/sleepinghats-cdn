...
`;class on{async createProject(e,n,a){const i=await V(k.PROJECT_COMP_NAME,n,e),s=await Y(a);return await q(s,i,se.ENTITY_PROJECT_LINKER),i}async getAllProjects(e){return new Promise(async(n,a)=>{const i=await I(),s=new P;s.typeConnection=`the_entity_${se.ENTITY_PROJECT_LINKER}`,s.selectors=[`the_${k.PROJECT_COMP_NAME}_name`],s.name="first",s.inpage=100;const o=new P;o.conceptIds=[e||i.entityId],o.name="second",o.inpage=100,o.freeschemaQueries=[s],o.outputFormat=de,await ce(o,"").subscribe(async r=>{const l=r?.[0]?.data?.the_entity?.[`the_entity_${se.ENTITY_PROJECT_LINKER}`];n(l)}).catch(r=>{console.error(r);const l=r?.message;L.errorToast(`${l}`,4),a({error:!0,message:l})})})}async getProjectById(e){const a=(await I())?.token;let i=new G;i.composition=e,i.fullLinkers=[`the_${k.PROJECT_COMP_NAME}_name`,`the_${k.PROJECT_COMP_NAME}_main_page`,`the_${k.PROJECT_COMP_NAME}_${se.PROJECT_PAGE_LINKER}`],i.inpage=100;let s=new G;return s.fullLinkers=[`the_${k.PAGE_COMP_NAME}_title`,`the_${k.PAGE_COMP_NAME}_slug`,`the_${k.PAGE_COMP_NAME}_body`],s.inpage=100,await we([i,s],a)}async updateProjectField(e,n,a){const i=await I(),s=`the_${k.PROJECT_COMP_NAME}_${e}`;await pe(a,s);const o=await z(a),r=await ee(`${e}`,`${n}`,!1,i.userId,4,999);await q(o,r,e)}async deleteProject(e){try{return await Ce(e),!0}catch(n){return console.error("Failed to delete project:",n),!1}}async addPagesToProject(e,n){try{const a=await z(e);if(!a){console.error("Project not found");return}await pe(e,`the_project_${se.PROJECT_PAGE_LINKER}`);for(const i of n){const s=await z(i);if(!s){console.error("Page not found");return}await q(a,s,se.PROJECT_PAGE_LINKER)}await N.SyncDataOnline()}catch(a){console.error("Failed to add page to project:",a)}}async deletePageFromProject(e,n){try{const a=await z(e);if(!a){console.error("Project not found");return}const i=await z(n);if(!i){console.error("Page not found");return}const s=await Sn(a,i,"",`the_project_${se.PROJECT_PAGE_LINKER}`);if(!s){console.error("Connection not found");return}await Me(s?.[0]?.id),await N.SyncDataOnline()}catch(a){console.error("Failed to delete page from project:",a)}}async setMainPage(e,n){try{const a=await z(e);if(!a){console.error("Project not found");return}const i=await z(n);if(!i){console.error("Page not found");return}await pe(e,"the_project_main_page"),await q(a,i,"main_page"),await N.SyncDataOnline()}catch(a){console.error("Failed to set main page:",a)}}}let ge=!1;const cr=async()=>{if(ge)return;ge=!0;let t=new Qt;const e=await rn(t),n=e?.routesDir,a=e?.pagesDir;await ur(a,n),t.generateAsync({type:"blob"}).then(function(i){let s=document.createElement("a");s.href=URL.createObjectURL(i),s.download="my_files.zip",s.click()}),ge=!1},dr=async t=>{if(ge)return;ge=!0;let e=[{path:"/",linkLabel:"Home",content:"homeIndex",importFrom:"/home/home.index.ts"}],n=new Qt;const a=await rn(n),i=a?.widgetsDir,s=a?.pagesDir;i&&s&&await mr(t,s,i),a?.routesDir?.file("routes.ts",sn(e)),n.generateAsync({type:"blob"}).then(function(o){let r=document.createElement("a");r.href=URL.createObjectURL(o),r.download="my_files.zip",r.click()}),ge=!1},rn=async t=>{t.file("index.html",tr),t.file("vite.config.js",Zo),t.file(".gitignore",Xo),t.file("package.json",er),t.file("tsconfig.json",ar);let e=t.folder("public");if(!e){console.log("unable to create public folder");return}let n=t.folder("src");if(!n){console.log("unable to create src folder");return}n?.file("app.ts",nr),n?.file("style.css","");let a=n?.folder("environments");if(!a){console.log("unable to create environments folder");return}a?.file("environment.dev.ts",ir);let i=n?.folder("widgets");if(!i){console.log("unable to create widgets folder");return}let s=n?.folder("pages");if(!s){console.log("unable to create pages folder");return}let o=n?.folder("services");if(!o){console.log("unable to create services folder");return}o?.file("widget.service.ts",or),o?.file("drop.service.ts",rr),o?.file("validator.service.ts",lr);let r=n?.folder("routes");if(!r){console.log("unable to create routes folder");return}return r?.file("renderRoute.service.ts",sr),{srcDir:n,pagesDir:s,servicesDir:o,publicDir:e,environmentsDir:a,widgetsDir:i,routesDir:r}},ur=async(t,e)=>{if(!t&&!e)return;let n=[];const a=await I(),i=a?.entityId,s=a?.token,o=new G;o.composition=i,o.inpage=100,o.fullLinkers=["the_entity_s_page"];const r=new G;r.inpage=100,r.fullLinkers=["the_page_body","the_page_title","the_page_slug","the_page_font_family","the_page_font_size","the_page_width","the_page_type"];const l=await we([o,r],s);if(l?.data.the_entity?.the_entity_s_page?.length)for(let c=0;c<l?.data.the_entity?.the_entity_s_page?.length;c++){const u=l.data.the_entity.the_entity_s_page[c];let d={page:u?.data?.the_page?.the_page_body?.[0]?.id,title:u?.data?.the_page?.the_page_title?.[0]?.data?.the_title,slug:u?.data?.the_page?.the_page_slug?.[0]?.data?.the_slug,font_family:u?.data?.the_page?.the_page_font_family?.[0]?.data?.the_font_family,font_size:u?.data?.the_page?.the_page_font_size?.[0]?.data?.the_font_size,width:u?.data?.the_page?.the_page_width?.[0]?.data?.the_width,type:u?.data?.the_page?.the_page_type?.[0]?.data?.the_type};if(!d.page)continue;const g=`${d.slug.split("-").map((h,w)=>w===0?h.charAt(0).toLowerCase()+h.slice(1):h.charAt(0).toUpperCase()+h.slice(1)).join("")}`;n.push({path:`/${d.slug}`,content:`${g}Index`,importFrom:`/${d.slug}/${d.slug}.index.ts`});let m=t?.folder(d.slug);const p=`
...
`;o?.file("home.index.ts",r),s?.css&&o?.file("home.style.css",s?.css)},gr=async t=>{if(ge)return;ge=!0;let e=[{path:"/",linkLabel:"Home",content:"homeIndex",importFrom:"/home/home.index.ts"}],n=new Qt;const a=await rn(n),i=a?.widgetsDir,s=a?.pagesDir;let o="";i&&s&&(o=await pr(t,s)),a?.routesDir?.file("routes.ts",sn(e)),n.generateAsync({type:"blob"}).then(function(r){let l=document.createElement("a");l.href=URL.createObjectURL(r),l.download=`${o||"page"}.zip`,l.click()}),ge=!1},pr=async(t,e)=>{const n=await I(),a=n?.entityId,i=n?.token,s=await na(t,i,!0),o=new G;o.composition=a,o.inpage=100,o.fullLinkers=["the_entity_s_page"];const r=new G;r.inpage=100,r.composition=t,r.fullLinkers=["the_page_body","the_page_title","the_page_slug","the_page_font_family","the_page_font_size","the_page_width","the_page_type"];const c=(await we([o,r],i))?.data?.the_entity?.the_entity_s_page?.filter(p=>p?.id==t),u=c?.[0]?.data?.the_page?.the_page_body?.[0]?.id,d=c?.[0]?.data?.the_page?.the_page_slug?.[0]?.data?.the_slug;let g=e?.folder("home");const m=`
    import * as tsccs from "mftsccs-browser";
    ...
    `}let yn,lt;async function Tr(t,e){yn=t;const n=Er(e,typeof t=="object"?0:t),a=document.createElement("div");a.innerHTML=n;const i=a.querySelector("#folder-widget-container");document.querySelector("body #app")?.appendChild(i),await me("folder-widget-container");const s=await I();if(!s)return;let o,r,c=(await nt(yn,s.token))?.data?.the_widget?.the_widget_name?.[0]?.data?.the_name;e===!0?(o=document.getElementById("folder-widget-container")?.querySelector("input#save-to-folder-name"),o&&(o.value=c),r=document.getElementById("folder-widget-container")?.querySelector("textarea#save-to-folder-description")):(o=document.getElementById("folder-widget-container")?.querySelector("input#folder-name"),r=document.getElementById("folder-widget-container")?.querySelector("textarea#description"));const u=document.getElementById("widgetFolder");lt=await $e(J.folder_widget,u,{widgetName:o.value,widgetDescription:r?.value,widgetId:yn,showCheckbox:!0,showFiles:!1})}async function Sr(t,e,n,a){t.preventDefault(),console.warn("asdfads",lt.selectedConceptIds);let i=[];lt&&lt?.selectedConceptIds?.length>0&&(i=lt.selectedConceptIds||[]);try{L.infoMessageToast("Saving Widget...",0),await je();const s=document.getElementById("folder-widget-container"),o=s?.querySelector('button[type="submit"]');o.textContent="Saving";const r=s?.querySelector("input"),l=s?.querySelectorAll("button"),u=(document.getElementById("save-to-folder-name")||document.getElementById("folder-name")).value;l&&l.forEach(T=>{T.disabled=!0}),r&&(r.disabled=!0),await it();let d;if(!a){R.staticElement?.removeAttribute("draggable");const T=document.querySelectorAll(".edited-widget-container");T?.length&&T.forEach(async _=>{if(_.classList.replace("edited-widget-container","added-widget-container"),f.editedWidgets[_.id]){const C=await Le(_,"div.mftsccs-marking-element");f.editedWidgets[_.id].html=C.innerHTML}});const b=await ki();f.staticWidgetTree.html=b;const y=await f.saveWidget(u);d=await Y(y.id),n=d.id}const g=await I(),m=document.getElementById("folder-widget-container")?.querySelector("input#save-to-folder-name")?.value||document.getElementById("folder-widget-container")?.querySelector("input#folder-name")?.value,p=document.getElementById("folder-widget-container")?.querySelector("textarea#save-to-folder-description")?.value,h={title:m,source_url:window.location.href,widget_id:n,short_desc:p,note:"",data_type:"widget",created_on:Date.now()};if(i.length==0){const T=g?.userConcept;d=await Y(Number(n));let b={...h,boom_folder:{name:"",parent:T}};const y=await Oe({boomgpt:b},void 0,void 0,void 0,g.userId,4);await ga(await Y(Number(T)),y,"the_boom_folder_s_contain"),await Q(y,d,"the_refer")}else if(i?.length>0)for(let T=0;T<i.length;T++){const b=i[T];d=await Y(Number(n));const y=await Y(Number(b));if(!y)throw new Error("Selected folder concept not found!");let _={...h,boom_folder:{name:y.name||"",parent:b}};const C=await Oe({boomgpt:_},void 0,void 0,void 0,g.userId,4);await ga(await Y(Number(b)),C,"the_boom_folder_s_contain"),await Q(C,d,"the_refer")}await N.SyncDataOnline(),document.getElementById(e)?.querySelector("form")?.reset(),be("folder-widget-container"),document.querySelector(".infoMessageToast")?.remove(),L.successfullToast("Widget saved successfully!"),a||(await D.renderSavedWidgets(),Ue(t,Number(n)))}catch(s){document.querySelector(".infoMessageToast")?.remove(),L.errorToast("Error in saving widget"),Yn(s,"There is an error on saving widget")}}class _i extends bt{mainConcept=0;mainData;data=0;phonebooks=[];the_params;component_heading;saveButtonHTML;saveBoomButtonHTML;previewButtonHTML;tryAIButtonHTML="";responsiveButtonHTML="";_pageBuilderService;constructor(e){super(),this.component_heading="Widget Builder Panel",this.saveButtonHTML="",this.saveBoomButtonHTML="";const n=I();if(n&&(this.saveBoomButtonHTML=`
...
`}before_render(){this.render()}after_render(){if(this.the_params){document.getElementById("fspage-save-button").addEventListener("click",Gn);const n=this.the_params.data?.the_page_body?.id;if(document.querySelector(".content-target").classList.add("fspage"),!n){tn();return}this._pageBuilderService.LoadPageWidget(Number(n))}}getHtml(){return window.openSaveWidgetModal=ea,window.saveWidget=xi,window.updateWidget=ta,window.newWidget=qe,window.previewWidget=Dr,window.downloadWebsite=cr,window.moveUp=Ee.moveUp,window.moveDown=Ee.moveDown,window.copyElement=Ee.copyElement,window.editWidget=Ee.editWidget,window.openConvertModal=qr,window.convertToWidget=Nr,window.updatePage=Qn,window.previewPage=_r,window.undoAction=H.undoAction,window.redoAction=H.redoAction,window.saveWidgetToFolderBtn=Tr,window.openPromptModal=_o,window.refineAIData=xn,`
    ...
    `,document.querySelector("body #app")?.appendChild(n),me("create-project-dialog"),"Create Project Modal"}async renameProjectModal(e,n){document.getElementById("rename-project-dialog")?.remove();const i=document.createElement("dialog");return i.setAttribute("id","rename-project-dialog"),i.classList.add("col-md-3"),i.innerHTML=`
    ...
    `,document.querySelector("body #app")?.appendChild(i),me("rename-project-dialog"),"Rename Project Modal"}async getProjectPagesListHTML(e){try{const n=await this._projectsService.getProjectById(e),a=n?.data?.the_project?.[`the_project_${se.PROJECT_PAGE_LINKER}`],i=n?.data?.the_project?.the_project_main_page?.[0]?.id;return a?.length?a?.map(o=>{let r=o?.data?.[`the_${k.PAGE_COMP_NAME}`]?.[`the_${k.PAGE_COMP_NAME}_title`]?.[0]?.data?.the_title;const l=i==o.id,c=l?'<span class="material-symbols-outlined home-icon">home</span>':"",u=l?"":`<li class="d-flex align-items-center justify-content-between" 
          ...
          `}catch(n){return console.error("error",n),n?.status===401&&xe(),`
            <li class="list-unstyled">
              <p class="text-center my-3 text-secondary">Sorry, couldn't connect to server.</p>
            </li>
...
.custom-menu-item {
  ...
  `}async function $r(t,e){if(t.preventDefault(),!(await _e.getInstance().getLoginInfo()).isAdmin)return"Sorry You Do Not Have Access To Create Element";const a=new FormData(t.target),i=Object.fromEntries(a);console.log(i,"update formValues");const s=await Li(a,e);await Zn("update-element-dialog");const o=await ln();if(s){const r=document.getElementById("created-elements");r?!o||o.trim()===""?r.innerHTML=`
    ...
    `}}async function me(t){document.getElementById(t).showModal()}async function be(t){const e=document.getElementById(t);e?.querySelector("form")?.reset(),e?.close(),e?.remove()}async function Wi(t,e){t?.stopPropagation();const n=document.getElementById(`menu-${e}`),a=n?.classList.contains("show");document.querySelectorAll(".dropdown-menu").forEach(s=>{s.classList.remove("show")}),n&&!a&&n.classList.add("show")}async function Cc(t){new jr,await me(t)}async function Lc(t,e,n){t.stopPropagation(),console.log("modalId",e),Wi(t,Number(n)),new Br(n)}async function Ac(t,e){Wi(t,e),confirm("Are you sure you want to delete the element?")&&(await Ce(e),L.successfullToast("The element deleted successfully")),console.warn("element Deleted sucessfully");const a=await ln(),i=document.getElementById("created-elements");i?!a||a.trim()===""?i.innerHTML=`
        <li class="list-unstyled">
          <p class="text-center my-3 text-secondary" >No Elements</p>
        </li>
  ...
  `,document.querySelector("body #app")?.appendChild(e),me("save-widget-dialog"),new Promise((n,a)=>{const i=e.querySelector("#save-widget-form");i.onsubmit=async o=>{o.preventDefault();try{const r=new FormData(i),c=Object.fromEntries(r).widgetName,u=o.target;if(!c?.trim()){const d=u.querySelector("#widgetName");d?.focus(),d.style.border="2px solid red",d.style.outline="none",d.addEventListener("input",()=>{d.style.border=""});return}await xi(o,c),be("save-widget-dialog"),n("Widget saved successfully")}catch(r){console.error("Error saving widget:",r),a("Error saving widget")}};const s=e.querySelector("button[type='button']");s.onclick=()=>{be("save-widget-dialog"),n("Widget saving canceled")}})}async function xi(t,e){t.preventDefault();try{L.infoMessageToast("Saving Widget...",0),await je();const n=document.getElementById("save-widget-dialog"),a=n?.querySelector('button[type="submit"]');a.textContent="Saving";const i=n?.querySelector("input"),s=n?.querySelectorAll("button");s&&s.forEach(d=>{d.disabled=!0}),i&&(i.disabled=!0),await it(),R.staticElement?.removeAttribute("draggable");const o=document.querySelectorAll(".edited-widget-container");o?.length&&o.forEach(async d=>{if(d.classList.replace("edited-widget-container","added-widget-container"),f.editedWidgets[d.id]){const g=await Le(d,"div.mftsccs-marking-element");f.editedWidgets[d.id].html=g.innerHTML}});const r=await ki();f.staticWidgetTree.html=r;const l=await f.saveWidget(e),c=await Y(l.id);be("save-widget-dialog"),en(),document.querySelector(".infoMessageToast")?.remove(),L.successfullToast("Widget saved successfully!"),await D.renderSavedWidgets(),Ue(t,c.id);return}catch(n){document.querySelector(".infoMessageToast")?.remove(),L.errorToast("Error in saving widget"),Yn(n,"There is an error on saving widget")}}async function ta(t,e,n){const a=document.getElementById("update-publish-widget-btn");try{const i=document.getElementById("update-widget-btn");a&&a.classList.add("disabled"),i&&(i.classList.add("disabled"),i.textContent="Updating...",L.infoMessageToast("Updating Widget...",0)),await it(),R.staticElement?.removeAttribute("draggable");const s=document.querySelectorAll(".edited-widget-container");s?.length&&s.forEach(async c=>{if(c.classList.replace("edited-widget-container","added-widget-container"),f.editedWidgets[c.id]){const u=await Le(c,"div.mftsccs-marking-element");f.editedWidgets[c.id].html=u.innerHTML}});const o=await Or();f.staticWidgetTree.html=o,await f.updateWidget(),en(),ye(),document.querySelector(".infoMessageToast")?.remove(),L.successfullToast("Widget Updated Successfully"),await D.renderSavedWidgets(),await D.renderBoomWidgets();const l=document.getElementById("widget-builder");l.textContent="Widget Builder Panel",await Ue(t,f.staticWidgetTree?.origin?f.staticWidgetTree?.origin:e),n&&n===!0&&await f.publish();return}catch(i){document.querySelector(".infoMessageToast")?.remove();const o=document.getElementById("update-widget-btn");o&&(o.classList.remove("disabled"),o.innerHTML=`
  ...
  `,document.body.appendChild(s);const o=s.querySelector("#pasteArea"),r=s.querySelector("#widgetThumbnail"),l=s.querySelector("#imagePreview img"),c=s.querySelector("#imagePreview"),u=s.querySelector("#pasteText");function d(p){const h=new FileReader;h.onload=w=>{l&&w.target?.result&&(l.src=w.target.result,c.classList.remove("d-none"),u.classList.add("d-none"))},h.readAsDataURL(p)}r?.addEventListener("change",p=>{const h=p.target.files?.[0];h&&d(h)});function g(p){const h=p.clipboardData?.items;if(h){for(let w=0;w<h.length;w++)if(h[w].type.indexOf("image")!==-1){const E=h[w].getAsFile();if(E&&r){const A=new DataTransfer;A.items.add(E),r.files=A.files,d(E);break}}}}o?.addEventListener("paste",p=>g(p)),s.addEventListener("paste",p=>g(p)),s.querySelector("#post-widget-form")?.addEventListener("submit",p=>{p.preventDefault(),gl(e,s)}),s.showModal()}async function gl(t,e){const n=document.querySelector(`#spinner-${t}`),a=e.querySelector('button[type="submit"]');try{a&&(a.disabled=!0,a.textContent="Posting...",L.infoMessageToast("Posting Widget to Public...",0)),n&&n.classList.remove("d-none"),await je();const i=await I(),s=document.getElementById("widgetPublicName").value,o=document.getElementById("widgetDescription").value,r=document.getElementById("widgetThumbnail");if(!r?.files?.length)throw console.error("File validation failed:",{inputExists:!!r,filesLength:r?.files?.length}),new Error("Please provide a thumbnail image");const l=r.files?.[0];if(!l||!r.value)throw new Error("Please provide a thumbnail image");const c=await Mi(l);if(!c.success)throw new Error("Failed to upload thumbnail");const u=await Te(t,i.token);if(u&&(await Pe(u.widgetId,"the_public_widget_s_widget",!0)).length){document.querySelector(".infoMessageToast")?.remove(),L.infoMessageToast("Widget is already public.");return}const d=document.getElementById("postAsTemplate")?.checked;let g=null;if(u)g=await z(u.widgetId);else{const A=await Y(t);g=await D.createWidgetCopy(A,i.userId,i.token)}const m=await ee("public_name",s,!1,i.userId,4,999),p=await ee("description",o,!1,i.userId,4,999),h=await ee("thumbnail",c.url||"no attachment",!1,i.userId,4,999);await q(g,p,"description"),await q(g,h,"thumbnail"),await q(g,m,"public_name");const w=await ee("public_widget","",!1,i.userId,4,999);if(await q(w,g,"s_widget"),d){const T=await ee("template_name",s,!1,i.userId,4,999),b=await ee("template_thumbnail",c.url||"no attachment",!1,i.userId,4,999);await q(g,b,"template_thumbnail"),await q(g,T,"template_name");const y=await ee("template_widget","",!1,i.userId,4,999);await q(y,g,"s_widget")}await N.SyncDataOnline(),await D.renderPublicWidgets(),await D.renderSavedWidgets(),d&&await D.renderTemplateLayouts(),e.close(),document.querySelector(".infoMessageToast")?.remove(),setTimeout(()=>{L.successfullToast(`Widget Posted to Public${d?" and Templates":""}`)},300)}catch(i){console.error("Error posting widget:",i),document.querySelector(".infoMessageToast")?.remove(),L.errorToast("Failed to post widget"),a&&(a.disabled=!1,a.textContent="Post Widget")}finally{n&&n.classList.add("d-none")}}async function Bn(t=""){try{const e=await I();if(!e)return`
    ...
    `}catch(e){console.error("error",e),e?.status===401&&xe()}}async function pl(t=""){try{const e=await I();if(!e)return`
        <li class="list-unstyled">
          <p class="text-center my-3 text-secondary">No Folder widget</p>
        </li>
      `;const a=(await Jr(t))[0]?.data?.the_folder_widget?.the_folder_widget_s_widget||[],i=await Promise.all(a.map(async l=>{const c=l?.data?.the_widget,d=c?.the_widget_folder_name?.data?.the_folder_name||`${c?.the_widget_name?.data?.the_name} ${c?.the_widget_version?.data?.the_version>1?"v"+c?.the_widget_version?.data?.the_version:""}`,g=c?.the_widget_description?.data?.the_description,m=c?.the_widget_thumbnail?.data?.the_thumbnail,p=c?.the_widget_timestamp?.data?.the_timestamp;return{id:l?.id,name:d,timestamp:p,description:g,thumbnail:m}}));if(!i?.length){const l=document.getElementById("folder-widget-container");return l&&(l.style.display="block",await $e(J.folder_widget,l)),`
        <li class="list-unstyled">
          <p class="text-center my-3 text-secondary">No Folder widget</p>
        </li>
