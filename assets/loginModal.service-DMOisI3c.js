import{y as I,C as M}from"./mftsccs-browser-C1xwC_1-.js";import{D as g,M as y,I as b,O as w,A as h}from"./index-Bl6yb6nn.js";class S{constructor(){this.init()}init(){const s=this.getHtml();document.querySelector("body #app")?.insertAdjacentHTML("afterend",s)}getHtml(){return`
...
<div id="user-details-modal" class="modal d-block" tabindex="-1" >
  ...
  `);const t=document.getElementById("user-details-modal");document.getElementById("cancel-btn").addEventListener("click",function(){t?.remove()});const e=document.getElementById("user-details-form"),n=document.getElementById("title"),r=document.getElementById("username-complete"),a=document.getElementById("title-error"),c=document.getElementById("username-error");e.addEventListener("submit",async p=>{p.preventDefault();const m=n.value.trim(),o=r.value.trim();let l=!0;m?a.textContent="":(a.textContent="Title is required.",l=!1),o?await C(o)?(c.textContent="The username already exists. Please choose another.",l=!1):c.textContent="":(c.textContent="Username is required.",l=!1),l&&(t?.remove(),d({title:m,username:o}))})})}async function C(d){try{return!!(await(await fetch(`${b.baseNodeUrl}/api/v1/username/${d}`)).json())?.data?.username}catch(s){return console.error("Error checking username:",s),!0}}function k(d){try{const t=d.split(".")[1].replace(/-/g,"+").replace(/_/g,"/"),e=decodeURIComponent(atob(t).split("").map(n=>`%${`00${n.charCodeAt(0).toString(16)}`.slice(-2)}`).join(""));return JSON.parse(e)}catch(s){return console.error("Invalid JWT token",s),null}}const B=async d=>{const t=`${b?.baseURL}/signin`;let e;const n=new Headers;n.append("Content-Type","application/json");try{const r=await fetch(t,{method:"POST",headers:n,body:JSON.stringify(d),redirect:"follow"}),a=await r.json();return r.ok?e={message:"success",status:!0,statusCode:200,data:a?.data}:e={message:"Invalid credentials",status:!1,statusCode:r.status,data:""},e}catch(r){return console.error("Sign in api error",r),e={message:"error",status:!1,statusCode:500,data:""},e}};async function H(d){d.preventDefault();const s=d.target,t=document.getElementById("login-error-message"),e=s.querySelector('button[type="submit"]');e.disabled=!0,e.innerHTML='<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Logging in...';try{const n=new FormData(s),r={email:n.get("email"),password:n.get("password")};s.querySelector("#remember-me")?.checked?(localStorage.setItem("remember-me","true"),localStorage.setItem("remembered-email",r.email)):(localStorage.removeItem("remember-me"),localStorage.removeItem("remembered-email"));const c=await B(r);if(c?.statusCode===200){await w(c);const p=await h.getInstance().getLoginInfo(),m=document.getElementById("login-info");m.innerHTML=p,g("login-dialog"),location.reload()}else t&&(t.innerHTML=`
          <div class="error-message">
            <span class="material-symbols-outlined">error</span>
            ${c?.message}
          </div>
