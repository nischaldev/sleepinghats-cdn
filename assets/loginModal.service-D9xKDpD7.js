import{eB as I,eH as M}from"./mftsccs-browser-fkDlgrMi.js";import{D as g,N as y,I as b,Q as w,A as h}from"./index-Vufosqdv.js";class S{constructor(){this.init()}init(){const s=this.getHtml();document.querySelector("body #app")?.insertAdjacentHTML("afterend",s)}getHtml(){return`
...
<div id="user-details-modal" class="modal d-block" tabindex="-1" >
  ...
  `);const t=document.getElementById("user-details-modal");document.getElementById("cancel-btn").addEventListener("click",function(){t?.remove()});const e=document.getElementById("user-details-form"),a=document.getElementById("title"),r=document.getElementById("username-complete"),n=document.getElementById("title-error"),c=document.getElementById("username-error");e.addEventListener("submit",async p=>{p.preventDefault();const m=a.value.trim(),o=r.value.trim();let l=!0;m?n.textContent="":(n.textContent="Title is required.",l=!1),o?await k(o)?(c.textContent="The username already exists. Please choose another.",l=!1):c.textContent="":(c.textContent="Username is required.",l=!1),l&&(t?.remove(),d({title:m,username:o}))})})}async function k(d){try{return!!(await(await fetch(`${b.baseNodeUrl}/api/v1/username/${d}`)).json())?.data?.username}catch(s){return console.error("Error checking username:",s),!0}}function B(d){try{const t=d.split(".")[1].replace(/-/g,"+").replace(/_/g,"/"),e=decodeURIComponent(atob(t).split("").map(a=>`%${`00${a.charCodeAt(0).toString(16)}`.slice(-2)}`).join(""));return JSON.parse(e)}catch(s){return console.error("Invalid JWT token",s),null}}const C=async d=>{const t=`${b?.baseURL}/signin`;let e;const a=new Headers;a.append("Content-Type","application/json");try{const r=await fetch(t,{method:"POST",headers:a,body:JSON.stringify(d),redirect:"follow"}),n=await r.json();return r.ok?e={message:"success",status:!0,statusCode:200,data:n?.data}:e={message:"Invalid credentials",status:!1,statusCode:r.status,data:""},e}catch(r){return console.error("Sign in api error",r),e={message:"error",status:!1,statusCode:500,data:""},e}};async function H(d){d.preventDefault();const s=d.target,t=document.getElementById("login-error-message"),e=s.querySelector('button[type="submit"]');e.disabled=!0,e.innerHTML='<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Logging in...';try{const a=new FormData(s),r={email:a.get("email"),password:a.get("password")};s.querySelector("#remember-me")?.checked?(localStorage.setItem("remember-me","true"),localStorage.setItem("remembered-email",r.email)):(localStorage.removeItem("remember-me"),localStorage.removeItem("remembered-email"));const c=await C(r);if(c?.statusCode===200){await w(c);const p=await h.getInstance().getLoginInfo(),m=document.getElementById("login-info");m.innerHTML=p,g("login-dialog"),location.reload()}else t&&(t.innerHTML=`
          <div class="error-message">
            <span class="material-symbols-outlined">error</span>
            ${c?.message}
          </div>
        `)}catch(a){console.error("Login error:",a),t&&(t.innerHTML=`
        <div class="error-message">
          <span class="material-symbols-outlined">error</span>
          An error occurred during login. Please try again.
        </div>
      `)}finally{e.disabled=!1,e.innerHTML="Log In"}}async function D(d){g("login-dialog"),document.getElementById("signup-dialog")?.remove(),new q;const t=document.getElementById("signup-dialog");if(t){t.querySelectorAll(".password-toggle").forEach(n=>{n.addEventListener("click",()=>{const p=n.closest(".password-field-wrapper")?.querySelector("input");p.type==="password"?(p.type="text",n.textContent="visibility"):(p.type="password",n.textContent="visibility_off")})}),t.querySelector(".close-button")?.addEventListener("click",()=>{g("signup-dialog"),document.body.classList.remove("modal-open")}),t.addEventListener("click",n=>{const c=t.getBoundingClientRect();(n.clientX<c.left||n.clientX>c.right||n.clientY<c.top||n.clientY>c.bottom)&&(g("signup-dialog"),document.body.classList.remove("modal-open"))}),t.querySelector(".login-trigger")?.addEventListener("click",()=>{L()});const r=t.querySelector("form");r&&r.addEventListener("submit",n=>{n.preventDefault(),n.stopPropagation(),P(n)})}y("signup-dialog"),document.body.classList.add("modal-open"),E("google-sign-in-button-signup")}async function P(d){d.preventDefault();const s=document.getElementById("signup-error-message");s.querySelector("p")?.remove();const e=d.target,a=e.elements;let r=!0;for(let i=0;i<a.length;i++)a[i].style.border="";for(let i=0;i<a.length;i++){const f=a[i];f.required&&f.value.trim()===""&&(f.style.border="1px solid red",r=!1)}const n=a.password.value,c=a["confirm-password"].value;if(n!==c){r=!1;const i=document.getElementById("confirm-password");i.style.border="1px solid red";const f=document.createElement("p");f.classList.add("text-danger"),f.innerText="Passwords do not match!",s.appendChild(f)}const p=document.getElementById("privacyPolicy"),m=document.getElementById("privacyPolicyError");if(p.checked?m.style.display="none":(r=!1,m.style.display="block"),!r)return;const o=document.getElementById("signup-btn");o.disabled=!0,o.innerHTML='<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Submitting';const l=new FormData(e),u=Object.fromEntries(l);for(let i=0;i<a.length;i++)a[i].disabled=!0;const v={entityType:u?.type,title:u?.title,fname:u?.firstname,lname:u?.lastname,timestamp:new Date().toISOString(),username:u?.username,email:u?.email,password:u?.password,application:"boomconsole"};try{(await M(v)).success?(g("signup-dialog"),L()):s.innerHTML='<p class="text-danger">Signup failed. Please try again.</p>'}catch(i){console.error("Signup API error:",i),s.innerHTML=`<p class="text-danger">${i.message}</p>`}for(let i=0;i<a.length;i++)a[i].disabled=!1;o.disabled=!1,o.innerHTML="Save Now"}export{S as L,q as S,P as a,L as o,E as r,H as s};
